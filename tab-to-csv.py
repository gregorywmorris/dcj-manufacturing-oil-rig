...
for files in os.listdir(path=data_location):
    ...
    df = pd.DataFrame()
    for i in range(60):
        df2 = pd.DataFrame(columns=[file_name])  # Add columns and index, will get index error otherwise

        df2[file_name] = pd.read_table(f'./data/{files}', index_col=False, delim_whitespace=True, header=None, usecols=[i], encoding='latin-1')
        ...
        df2['profile'] = range(1,1+df2.shape[0])
